...
MOCK_CONFIG_SCORES = {
    "cards": {
        "scored": {
            "sections": {
                "scored_sub_section": {
                    "cases": ["test_number", "test_parametrized"],
    ...
    result1 = MemoteResult(
        {
            ...
            "tests": {
                "test_parametrized": {
                    ...
                    "duration": {"parameter1": 0.12, "parameter2": 0.32},
                    "format_type": "percent",
                    "message": {
                        "parameter1": "Some Message 1",
                        "parameter2": "Some Message 2",
                    },
                    "metric": {"parameter1": 0.5, "parameter2": 0.9},
                    "result": {"parameter1": "failed", "parameter2": "failed"},
    ...
    result2 = MemoteResult(
        {
            ...
            "tests": {
                "test_parametrized": {
                    ...
                    "duration": {"parameter1": 0.2, "parameter2": 0.1},
                    "format_type": "percent",
                    "message": {
                        "parameter1": "Some Message 1",
                        "parameter2": "Some Message 2",
                    },
                    "metric": {"parameter1": 1.0, "parameter2": 0.0},
                    "result": {"parameter1": "failed", "parameter2": "failed"},
                    "summary": "Some description of the test",
                    "title": "Parametrized Test",
                },
...
def test_structure(mock_history_manager):
    ...
    assert set(results["score"]["total_score"].keys()) == set(
        ["history", "format_type"]
    )
    assert set(results["score"]["total_score"]["history"][0]) == set(
        ["commit", "metric", "branch"]
    )
    assert set(
        results["tests"]["test_parametrized"]["history"]["parameter1"][0]
    ) == set(["commit", "metric", "branch", "data", "result"])
    assert set(results["tests"]["test_number"]["history"][0]) == set(
        ["commit", "metric", "branch", "data", "result"]
    )
...
def test_score_param(mock_history_manager):
    ...
    history = mock_history_manager
    score_collection = HistoryReport(history, MOCK_CONFIG_SCORES).result["score"]
